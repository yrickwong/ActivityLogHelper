...
object PermissionUtils {
    ...
    fun checkFloatWindowPermission(context: Context): Boolean {
        return when {
            ...
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> !RomUtils.isDomesticSpecialRom() || Settings.canDrawOverlays(
                context
            )
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT -> //AppOpsManager添加于API 19
                checkOps(context)
    ...
    private fun checkOps(context: Context): Boolean {
        try {
            ...
            */
            ...
            arrayOfObject1[2] = LogHelperApplication.getInstance().packageName
     ...
     * @param context
     */
    private fun applyCommonPermission(context: Context, consumer: IConsumer<Intent>) {
        try {
    ...
    private fun applyMiuiPermission(context: Context) {
        val versionCode = RomUtils.getMiuiVersion()
        if (versionCode == 5) {
    ...
    fun applyAuthorizePermission(context: Context, consumer: IConsumer<Intent>) {
